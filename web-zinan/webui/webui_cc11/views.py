...
def process_query(request):
    ...
    stop_and_remove_containers("news-producer-shihan")
    ...
    f"sudo docker run --network=host "
    f"-e 'BOOTSTRAP_SERVERS=129.114.27.101:30000' "
    f"-e 'SEARCH_QUERY={query}' "
    "news-producer-shihan"
    )
    result1=subprocess.run(command_reddit,shell=True,capture_output=True,text=True) 
    result2=subprocess.run(command_news,shell=True,capture_output=True,text=True) 
